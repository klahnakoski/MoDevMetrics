...
function translate(){
	if (translated) return; // run this only once
	translated = true;
	var body = document.getElementsByTagName("body")[0];
	if (translateASCIIMath) AMprocessNode(body, false);
	if (translateLaTeX) LMprocessNode(body);
}
...
var CONST = 0, UNARY = 1, BINARY = 2, INFIX = 3, LEFTBRACKET = 4, RIGHTBRACKET = 5, SPACE = 6, UNDEROVER = 7, DEFINITION = 8, LEFTRIGHT = 9, TEXT = 10, BIG = 11, LONG = 12, STRETCHY = 13, MATRIX = 14;
; // token types
...
//st = st.replace(/<\/?font.*?>/gi,""); // do this only in amath...endamath
	...
	st = st.replace(/(?:\\begin{a?graph}|\bagraph|\(:graph\s)((.|\n)*?)(?:\\end{a?graph}|enda?graph|:\))/g, function(s, t){
		return "<table><tr><td><div class=\"ASCIIsvg\"><embed class=\"ASCIIsvg\" src=\"" + dsvglocation + "d.svg\" wmode=\"transparent\" script=\'" + t.replace(/<\/?(br|p|pre)\s?\/?>/gi, "\n") + "\'/></div></td></tr></table>"
	});
...
defaultheight = 200;   // in pixels
var defaultxmin = -5.5;
defaultxmax = 5.5;     // in usercoords
var defaultborder = 0;
border = defaultborder; // in pixel
...
//      node.setAttribute("style","display:block");
				...
				if (isIE) node2.onclick = function(){
					updatePicture(this)
				};
