					...
					"requests" :[
					{
						"query" : {
							"query": {
                           	...
                           	"filter" : {
                                "and":[
                                    { "terms" : { "status_whiteboard.tokenized" : ["snappy:p1"]} },
                                    {"not" : {
...

                        {
                            "query" : {
                                "query": {
                                   ...
                                   "filter" : {
                                    "and":[
                                        { "terms" : { "status_whiteboard.tokenized" : ["snappy:p2"]} },
                                        {"not" : {
                                                "terms" : { "bug_status" : ["resolved", "verified", "closed"] }
                                            }
                                        }
...

                        {
                            "query" : {
                                "query": {
                                   ...
                                   "filter" : {
                                    "and":[
                                        { "terms" : { "status_whiteboard.tokenized" : ["snappy:p3"]} },
                                        {"not" : {
                                                "terms" : { "bug_status" : ["resolved", "verified", "closed"] }
                                            }
                                        }
...
//					"showSeries" : ["open Snappy bugs", "relative scope"],
					"chartTitle" : "Snappy - Open Bug Breakdown",
					"canvas" : "chart",
					"originZero" : true,
