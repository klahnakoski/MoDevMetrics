...
function getDateFromFormat(val, format, isPastDate) {
	...
	while (formatIndex < format.length) {
		...
		if (token=="yyyy" || token=="yy" || token=="y") {
			...
			if(dayOfMonth==null||(dayOfMonth<1)||(dayOfMonth>31)){return 0;}
			...
			if (val.substring(valueIndex,valueIndex+2).toLowerCase()=="am") {ampm="AM";
			}else if (val.substring(valueIndex,valueIndex+2).toLowerCase()=="pm") {ampm="PM";
			}else{return 0;}
			valueIndex+=2;
		}else if (token.trim()==""){
			while(val.charCodeAt(valueIndex)<=32) valueIndex++;
		}else{
			if (val.substring(valueIndex,valueIndex+token.length)!=token) {return 0;
			}else{valueIndex+=token.length;}
		}//endif
	}
...
DatePicker.prototype = {
	...
	defaults : {
		...
		dateFilter				: function(date){return false;},
		id_datepicker			: null,
    ...
    initialize : function (args) {
        ...
        if (this.args.enableCloseOnBlur) {
            Event.observe(this.args.id, 'blur', function (
                e
                ) {
                if (!this.args.closeTimer) this.args.closeTimer = this.close.bind(this).delay(1);
            }.bindAsEventListener(this));
            Event.observe(this._div, 'click', function (
                e
                ) {
