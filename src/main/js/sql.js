...
SQL.prototype.calc=function(query){
	...
	for(var i in query.from){
		...
		if (select.length==0){
			for(var t in allElements) SQL.addResultToOutput(allElements[t], indexedOutput, select, facets);
		}else{
			for (s in select){
				var ss=select[s];
				var v=ss.calc(row);
...
SQL.aggregate.join=function(column){
	...
	column.compare=SQL.valueCompare;
};
...
SQL.aggregate.average=function(select){
	...
	select.compare=SQL.valueCompare;
};
...
//	};//method
	...
	column.compare=function(a, b){
		return SQL.valueCompare(a.value, b.value);
	};//method
...
SQL.order=function(data, ordering, columns){
	...
	var totalSort=function(a, b){
		for(var o in ordering){
			var diff=columns[ordering[o]].compare(a[ordering[o]], b[ordering[o]]);
